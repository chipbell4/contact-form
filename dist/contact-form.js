var ContactForm = function(options) {
	var defaults = {
		 ...
		 * validate: A regex to match, or a function that returns true when the field is valid
	...
	var validate_field = function(field) {
		...
		// If the field's validate property is a function, check that the function returns true
		if(field.validate instanceof Function) {
			return field.validate( field_value(field) );
		}
		// If the field's validate property is a RegExp, use the test method
		else if(field.validate instanceof RegExp) {
			return field.validate.test( field_value(field) );
		}	
		// otherwise, return the property, treating it as a boolean
		else {
